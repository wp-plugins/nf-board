function pwFindSubmit(){
	jQuery(".pw-find-btn").addClass("disabled").hide();
	...
	jQuery.ajax({
		...
		dataType: "text",
		data: {
			action: "pw_find_action",
			formData: jQuery('#pwfindForm').serialize()
		},
		success: function(data) {
			...
			if(res == "success"){
				...
				jQuery("#result_msg").html("아이디를 입력해주세요.");
				jQuery("#user_id").focus();
			}else if(res == "empty email"){
				jQuery("#result_msg").show();
				jQuery("#result_msg").html("이메일을 입력해주세요.");
				jQuery("#email").focus();
			}else if(res == "not form email"){
				jQuery("#result_msg").show();
				jQuery("#result_msg").html("이메일 형식이 올바르지 않습니다.");
				jQuery("#email").focus();
			}
			jQuery(".pw-find-btn").removeClass("disabled").show();
...
jQuery(function() {
	jQuery("#user_id").keydown(function() {
		if(jQuery("#user_id").val() != ''){jQuery('#result_msg').empty().hide();}
		if(event.keyCode == 13) pwFindSubmit();
	});
	jQuery("#email").keydown(function() {
		if(jQuery("#email").val() != ''){jQuery('#result_msg').empty().hide();}
		if(event.keyCode == 13) pwFindSubmit();
	});
	jQuery(".pw-find-btn").click(function() {
		pwFindSubmit();
	});
});