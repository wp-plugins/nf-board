...
if($_GET['wrong'] == "1") $message = "오류 : 비밀번호를 바르게 입력해주세요.";
else if($_GET['wrong'] == "2") $message = "오류 : 비밀번호가 일치하지 않습니다.";
...
<body>
<form name="pwd_frm" method="post" action="<?php echo NFB_HOME_URL?>/?NFPage=board-pass-check-process">
...
<div style="padding-top:30px;">
	<div class="pw_box">
		...
		<fieldset>
			...
			<p id="error_box" class="open_alert" style="display:none;"><?php if(!empty($_GET['wrong'])) echo $message; else echo "비밀번호를 입력해주세요.";?></p>
			<p class="pw_btn_w"><a href="javascript:;" onclick="check();"><input title="확인" class="pw_box_btn" type="button" value="확인" /></a>&nbsp;<a href="javascript:;" onclick="location.href='<?php echo NFB_SITE_URL?>?page_id=<?php echo $_GET['page_id']?>&<?php echo build_param($_GET['bname'], "list", "", $_GET['page'], $_GET['keyfield'], $_GET['keyword'], $_GET['search_chk'], $_GET['cate'])?>';"><input title="취소" class="pw_box_btn" type="button" value="취소" /></a></p>
		</fieldset>
	</div>
</div>
</form>
